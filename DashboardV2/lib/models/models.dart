...
class Assignment {
  ...
  Assignment copyWith({bool? completed, bool? inProgress, String? details, DateTime? reminderTime}) =>
      Assignment(
        id: id, name: name, course: course, dueDate: dueDate, priority: priority,
  ...
  Map<String, dynamic> toJson() => {
        ...
        'due': dueDate.toIso8601String(), 'priority': priority,
        'completed': completed, 'inProgress': inProgress, 'details': details,
...
class ScheduleItem {
  ...
  ScheduleItem({String? id, required this.name, required this.day, required this.time, required this.location})
      : id = id ?? _uuid.v4();

  ScheduleItem copyWith({String? name, String? day, String? time, String? location}) =>
      ScheduleItem(id: id, name: name ?? this.name, day: day ?? this.day,
  ...
  factory ScheduleItem.fromJson(Map<String, dynamic> j) => ScheduleItem(
        ...
        name: j['name'], day: j['day'], time: j['time'], location: j['location'] ?? '',
      );
...
class Note {
  ...
  factory Note.fromJson(Map<String, dynamic> j) => Note(
        id: j['id']?.toString() ?? _uuid.v4(),
        title: j['title'], content: j['content'], branch: j['branch'] ?? 'general',
        date: j['date'] != null ? DateTime.tryParse(j['date']) ?? DateTime.now() : DateTime.now(),
      );

  Map<String, dynamic> toJson() =>
      {'id': id, 'title': title, 'content': content, 'branch': branch, 'date': date.toIso8601String()};
}
...
class Formula {
  ...
  Formula({String? id, required this.name, required this.formula,
      required this.desc, required this.category, this.isCustom = false})
      : id = id ?? _uuid.v4();
  ...
  factory Formula.fromJson(Map<String, dynamic> j) => Formula(
        id: j['id']?.toString() ?? _uuid.v4(),
        name: j['name'], formula: j['formula'], desc: j['desc'] ?? '',
        category: j['category'], isCustom: j['isCustom'] ?? true,
      );

  Map<String, dynamic> toJson() => {
        'id': id, 'name': name, 'formula': formula,
        'desc': desc, 'category': category, 'isCustom': isCustom,
      };
}

// ── Collection (unified hub tile) ────────────────────────────────
class Collection {
  final String id;
  final String title;
  final String emoji;
  final String type; // 'formula' | 'note' | 'email' | 'custom'

  Collection({String? id, required this.title, required this.emoji, required this.type})
      : id = id ?? _uuid.v4();

  factory Collection.fromJson(Map<String, dynamic> j) => Collection(
        id: j['id']?.toString() ?? _uuid.v4(),
        title: j['title'], emoji: j['emoji'] ?? '📁', type: j['type'] ?? 'custom',
      );

  Map<String, dynamic> toJson() => {'id': id, 'title': title, 'emoji': emoji, 'type': type};
}
...
class AppCategory {
  ...
  factory AppCategory.fromJson(Map<String, dynamic> j) =>
      AppCategory(id: j['id'] ?? _uuid.v4(), name: j['name'], emoji: j['emoji'] ?? '📁');
...
final List<Formula> defaultFormulas = [
  ...
  Formula(id: 'f16', name: 'Power', formula: 'P = W/t = Fv', desc: 'Rate of doing work', category: 'energy', isCustom: false),
